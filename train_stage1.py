...
from lib.network_1 import HumanModel
...
from lib.modules.dino import DINOv2
...
class Trainer:
    def __init__(self, cfg_file):
        ...
        if self.cfg.restore_ckpt:
            logging.info(f"Loading checkpoint from {self.cfg.restore_ckpt}")
        ...
        self.dino = DINOv2().cuda()
        set_requires_grad(self.dino, False)
    ...
    def train_depth_refine(self):
        ...
        for _ in progress_bar:
            ...
            depth_loss = self.model(data, False)
            refined_depth, pred_depth = depth_loss[1], depth_loss[2]
            refined_feat = self.dino(refined_depth)
            with torch.no_grad():
                pred_feat = self.dino(pred_depth)
            semantic_loss = (refined_feat - pred_feat).abs().mean()
            loss =depth_loss[0]+semantic_loss
    ...
    def train(self):
        ...
        for _ in progress_bar:
            ...
            img_pred, mask_pred, depth_pred,depth_loss = self.model(data)

            img_gt = data['target']['image'].cuda()
            mask_gt = data['target']['mask'].cuda()

            Ll1 = l1_loss(img_pred, img_gt)
            mask_loss = l2_loss(mask_pred, mask_gt)

            Lssim = 1.0 - ssim(img_pred, img_gt)

            loss = 0.8 * Ll1 + 0.2 * Lssim + mask_loss 
    ...
    def run_eval(self):
        ...
        for idx in range(self.len_val):
            ...
            with torch.no_grad():
                # img_pred, depth_pred = self.model(data, self.total_steps,True)
                img_pred= self.model(data, self.total_steps)[0]
                img_gt = data['target']['image'].cuda()

                psnr_value = psnr(img_pred, img_gt).mean().double()
                psnr_list.append(psnr_value.item())
                # tmp_novel = img_pred[0, 0].detach()
